...
| 종류 | 파일 수 | 테스트 수 | 내용 |
|------|--------|----------|------|
| 유닛 | 14 | 196 | 게임 로직 + 스토어 + 비주얼 헬퍼 + 큐 애니메이션 + ErrorBoundary |
| E2E | 3 | 14 | 세팅/전투(커스텀스킬·난이도별·MP소진)/풀플로우 |

커버리지: Stmts **99.76%**, Branch **98.3%**, Funcs **99.04%**, Lines **99.72%** (핵심 로직 `lib/` · `stores/` 100%)
