...
- 테스트가 즉각적 (213개 유닛 테스트, < 1초)
- 로직 변경 시 UI 변경 불필요 (관심사 분리)
- 모든 분기 조건을 결정적으로 검증 가능

> 대안: 스토어 내에 로직을 직접 작성하면 파일 수는 줄지만, 테스트 시 항상 스토어를 거쳐야 하므로 설정 비용이 높아집니다. 로직이 복잡한 게임 도메인에서는 순수 함수 분리가 테스트 속도와 유지보수 모두에서 유리합니다.

### 2. 명시적 상태 머신

전투 흐름을 `nextPhase(current, context)` 순수 함수로 정의했습니다. if/else 분산 대신 명시적 전이 테이블로 관리하여 전이 버그를 원천 차단합니다.

> 대안: XState 같은 상태 머신 라이브러리도 고려했으나, 전이 경로가 6개뿐인 이 규모에서는 순수 함수 하나로 충분합니다. 외부 의존성 없이 동일한 안전성을 확보합니다.

### 3. 액션 큐 패턴

한 라운드의 결과(선공 행동 → 후공 행동)를 큐에 적재하고, UI가 순차적으로 소비하며 연출합니다. 이를 통해 동기적 게임 로직과 비동기 UI 연출을 깔끔하게 분리합니다.

> 대안: 로직 자체를 비동기(setTimeout 체인)로 구현하면 큐 없이도 연출이 가능하지만, 게임 결과의 결정론적 검증이 불가능해집니다. 큐 패턴은 "로직은 동기, 연출은 비동기"라는 원칙을 지킵니다.

### 4. rng 의존성 주입

적 AI의 랜덤 함수를 파라미터로 주입합니다 (`decideEnemyAction(context, rng = Math.random)`). 프로덕션에서는 기본값 사용, 테스트에서는 고정 값 주입으로 결정적 테스트를 보장합니다.

> 대안: `vi.spyOn(Math, 'random')` 으로도 테스트는 가능하나, 글로벌 상태를 건드려 테스트 간 격리가 깨질 수 있습니다. 파라미터 주입은 1줄 추가로 완벽한 격리를 보장합니다.

### 5. 동시 선언 방어 모델

양쪽의 행동 선택은 **동시에 이루어지는 것**으로 처리합니다. 라운드 시작 시 양측의 방어 여부를 먼저 결정한 뒤 선공→후공 순서로 실행하므로, 후공이 방어를 선택하면 같은 라운드 선공의 공격에도 방어가 적용됩니다. 방어 상태는 해당 라운드에서만 유효하며 다음 라운드로 이월되지 않습니다.

> 대안: 순차 실행 모델(선공 행동 → 후공 행동)에서는 후공의 방어가 선공 공격에 적용되지 않아 후공에게 방어가 사실상 무의미해집니다. 동시 선언 모델은 행동 순서와 무관하게 양쪽 모두에게 방어의 가치를 보장합니다.
...
- **빌드 사이즈**: 304KB (gzip 91KB) — React+Zustand+Zod 포함

## 테스트

| 종류 | 파일 수 | 테스트 수 | 내용 |
|------|--------|----------|------|
| 유닛 | 17 | 213 | 게임 로직 + 스토어 + 비주얼 헬퍼 + 큐 애니메이션 + UI 렌더링 + ErrorBoundary |
| E2E | 4 | 21 | 세팅/전투(커스텀스킬·난이도별·MP소진)/풀플로우/엣지케이스(20라운드 무승부·스킬 제한·방어 전략) |

커버리지: Stmts **99.77%**, Branch **97.16%**, Funcs **99.05%**, Lines **99.73%** (핵심 로직 `lib/` · `stores/` 100%)
